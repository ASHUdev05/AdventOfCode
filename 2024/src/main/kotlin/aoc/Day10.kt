...
class Day10 {
    private fun getNeighbors(point: Point, grid: List<List<Int>>): List<Point> {
        val (x, y) = point
        return listOf(
    ...
    private fun findTrailsFromPoint(
        ...
        val reachableNines = mutableSetOf<Point>()
        visited.add(start)
    ...
    private fun countDistinctTrails(
        current: Point,
        grid: List<List<Int>>,
        path: MutableSet<Point> = mutableSetOf(),
        currentHeight: Int = 0,
        memo: MutableMap<Point, Int> = mutableMapOf()
    ): Int {
        if (currentHeight == 9) {
            return 1
        }
